...
// Recap page
export const getMontlyRecap = async (uid) => {
  const q = query(
  ...
  snapshot.forEach((doc) => {
    ...
    const createdAt = data.date?.toDate?.();
    const monthKey = format(createdAt, "MMMM yyyy");

    if (!monthlyRecap[monthKey]) {
      monthlyRecap[monthKey] = 0;
    }

    monthlyRecap[monthKey] += data.nominal;
  });
